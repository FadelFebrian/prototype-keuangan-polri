<body class="skin-blue fixed-layout">
				...
				<div class="navbar-collapse">
					...
					<ul class="navbar-nav my-lg-0">
						...
						<li class="nav-item dropdown u-pro">
							<a class="nav-link dropdown-toggle waves-effect waves-dark profile-pic" href="" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><img src="<?php echo base_url();?>assets/images/users/1.jpg"
									...
									<table id="table" class="display nowrap table table-hover table-striped table-bordered" cellspacing="0" width="100%">
                                        ...
                                        foreach($pelaksanaan as $val){?>
											<tr>
												...
												<td align="center">
													<?php echo $val->kode_output;?>
												</td>
												<td>
													<?php echo $val->nama_output;?>
												</td>
												<td align="center">
													<?php echo $val->no_kontrak;?>
												</td>
												<td>
													<?php echo $val->uraian;?>
												</td>
												<td>
													<?php echo "Rp. ".number_format($val->amount_pelaksanaan);?>
												</td>

												<td>
													<img src="<?php echo base_url('assets/images/bukti_transaksi/'.$val->bukti_transaksi);?>" style="width:100px">
												</td>
												<td>
													<button class='btn btn-info' onclick="edit_data(<?php echo $val->id_pelaksanaan;?>)"><i class='fa fa-edit' aria-hidden='true'></i>Edit</button>
													<button class="btn btn-danger" onclick="delete_data(<?php echo $val->id_pelaksanaan;?>)"><i class="fa fa-trash"
						...
						<div class="modal-dialog modal-lg" role="document">
							<div class="modal-content block3">
								...
								<div class="modal-body">
									...
									<div class="form-group row">
										...
										<div class="col-md-10">
                                            ...
                                            foreach($kegiatan as $val){?>
												<option value="<?php echo $val->id_kegiatan;?>">
													<?php echo $val->kode_kegiatan;?>
												</option>
												<?php } ?>
											</select>
									...
									<div class="form-group row">
										<label class="col-md-2 col-form-label">No Output</label>
										<div class="col-md-10">
											<select name="kode_output" id="kode_output" class="form-control" onchange="autoisi()">
												<option value="Pilih Opsi" selected>--Pilih Kode Output--</option>
												<?php 
                                            foreach($output as $val){?>
												<option value="<?php echo $val->id_output;?>">
													<?php echo $val->kode_output;?>
												</option>
												<?php } ?>
											</select>
									...
									<div class="form-group row">
										<label class="col-md-2 col-form-label">Nama Output</label>
										<div class="col-md-10">
											<input type="text" name="nama_output" id="nama_output" class="form-control" placeholder="Nama Output"
											 readonly>
										</div>
									</div>
									<div class="form-group row">
									...
									<div class="form-group row">
										...
										<div class="col-md-10">
											<input type="text" name="amount_pelaksanaan" id="amount_pelaksanaan" class="form-control" placeholder="Amount">
										</div>
									</div>
								...
								<div class="modal-footer">
									<input type="hidden" name="id_pelaksanaan" id="id_pelaksanaan">
	...
	<script>
		...
		function edit_data(id_pelaksanaan) {
			save_method = 'update';
			$('#form')[0].reset();


			$.ajax({
				url: "<?php echo base_url();?>C_polres/getDataById/" + id_pelaksanaan,
				type: 'GET',
				dataType: 'JSON',
				success: function (data) {
					// console.log(data);
		...
		function delete_data(id_pelaksanaan) {
			if (confirm('Apakah Yakin Ingin Hapus Data Ini?')) {

				$.ajax({
					url: '<?php echo base_url();?>C_polres/deleteKegiatan/' + id_pelaksanaan,
					type: 'POST',
					dataType: 'JSON',
					success: function (data) {
						location.reload();
		...
		function auto_complete() {
			...
			$.ajax({
				...
				success: function (data) {
					// console.log(data['kode'][0].program);
		...
		function autoisi() {
			...
			$.ajax({
				url: '<?php echo base_url();?>C_polres/getKodeOutput',
				type: 'POST',
				dataType: 'JSON',
				data: {
				...
				success: function (data) {
					// console.log(data['kode'][0].program_kegiatan);
					$('#nama_output').val(data['output'][0].nama_output);
					// $('#amount').val(data['kode'][0].amount);
				}
			});
