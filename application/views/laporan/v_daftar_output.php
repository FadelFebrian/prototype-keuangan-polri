<body class="skin-blue fixed-layout">
									...
									<table id="myTable" class="display nowrap table table-hover table-striped table-bordered" cellspacing="0"
									 width="100%">
										<thead>
											<tr align="center">
												<th width="50px"> No </th>
                                                <th width="100px">Kode Output</th>
                                                <th width="100px">Nama Output</th>
												<th width="100px">Amount Pelaksanaan</th>
                                        ...
                                        foreach($output as $val){?>
											<tr>
												...
												<td align="center">
													<button class='btn btn-info' onclick="edit_data(<?php echo $val->id_output;?>)"><i class='fa fa-edit' aria-hidden='true'></i>Edit</button>
													<button class="btn btn-danger" onclick="delete_data(<?php echo $val->id_output;?>)"><i class="fa fa-trash"
							...
							<div class="modal-content block3">
								...
								<div class="modal-body">
									<div class="form-group row">
										...
										<div class="col-md-10">
											<select name="kode_kegiatan" id="kode_kegiatan" class="form-control" onchange="auto_complete()">
											<option value="pilih" selected='selected'>--Pilih Kode Kegiatan--</option>
												<?php 
                                    ...
                                    <div class="form-group row">
										...
										<div class="col-md-10">
											<input type="text" name="nama_kegiatan" id="nama_kegiatan" class="form-control" placeholder="Nama Kegiatan" readonly>
										</div>
									</div>
									<div class="form-group row">
										<label class="col-md-2 col-form-label">Kode Output</label>
										<div class="col-md-10">
											<input type="text" name="kode_output" id="kode_output" class="form-control" placeholder="Kode Output">
										</div>
									</div>
									<div class="form-group row">
	...
	<script>
		...
		function save_data() {
			...
			if (save_method == 'add') {
				url = '<?php echo base_url();?>C_pusat/tambahLaporanOutput';
			} else {
				url = '<?php echo base_url();?>C_pusat/updateLaporanOutput';
			}
			$.ajax({
				url: url,
				type: 'POST',
		...
		function edit_data(id_output) {
			save_method = 'update';
			$('#form')[0].reset();

			$.ajax({
				url: "<?php echo base_url();?>C_pusat/editLaporanOutput/" + id_output,
				type: 'GET',
				dataType: 'JSON',
				success: function (data) {
					$('[name="id_output"]').val(data.id_output);
					$('[name="kode_kegiatan"]').val(data.id_kegiatan);
					$('[name="nama_kegiatan"]').val(data.nama_kegiatan);
                    $('[name="kode_output"]').val(data.kode_output);
                    $('[name="nama_output"]').val(data.nama_output);
                    $('[name="uraian"]').val(data.nama_output);
					$('[name="amount_output"]').val(data.amount_output);
					$('#form-modal').modal('show');
		...
		function delete_data(id_output) {
			if (confirm('Apakah Yakin Ingin Hapus Data Ini?')) {

				$.ajax({
					url: '<?php echo base_url();?>C_pusat/deleteLaporanOutput/' + id_output,
					type: 'POST',
					dataType: 'JSON',
					success: function (data) {
						location.reload();
		...
		function auto_complete() {
			...
			$.ajax({
				url: '<?php echo base_url();?>C_pusat/getNamaKegiatan',
				data: {
		...
		function auto() {
			...
			$.ajax({
				url: '<?php echo base_url();?>C_pusat/getNamaUraian',
				data: {
