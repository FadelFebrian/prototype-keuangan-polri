<body class="skin-blue fixed-layout">
								...
								<div class="row">
									...
									<div class="col-3">
										...
										<div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
											<a class="dropdown-item" href="<?php echo base_url();?>C_pusat/getLaporanProgram"><?php echo $year->tahun;?></a>
										</div> -->

										<form action="<?php echo base_url();?>C_pusat/getLaporanProgram" method='POST'>
										...
										<button type='submit' class='btn btn-info' name='cari'><i class='fa fa-search'>Cari</i></button>	
										</form>
									</div>
								</div>
									 ...
									 width="100%">
                                        ...
                                        foreach($laporan as $val){?>
											<tr>
												...
												<td>
													<button class='btn btn-info' onclick="edit_data(<?php echo $val->id_program;?>)"><i class='fa fa-edit' aria-hidden='true'></i>Edit</button>
													<button class="btn btn-danger" onclick="delete_data(<?php echo $val->id_program;?>)"><i class="fa fa-trash"
							...
							<div class="modal-content block3">
								...
								<div class="modal-body">
									<div class="form-group row">
										...
										<div class="col-md-10">
											<input type="text" name="kode_program" id="kode_program" class="form-control" placeholder="Kode Program">
										</div>
									</div>
									<div class="form-group row">
	...
	<script>
		...
		function save_data() {
			...
			if (save_method == 'add') {
				url = '<?php echo base_url();?>C_pusat/tambahLaporanProgram';
			} else {
				url = '<?php echo base_url();?>C_pusat/updateLaporanProgram';
			}
			$.ajax({
				url: url,
				type: 'POST',
		...
		function edit_data(id_program) {
			save_method = 'update';
			$('#form')[0].reset();

			$.ajax({
				url: "<?php echo base_url();?>C_pusat/editLaporanProgram/" + id_program,
				type: 'GET',
				dataType: 'JSON',
				success: function (data) {
		...
		function delete_data(id_program) {
			if (confirm('Apakah Yakin Ingin Hapus Data Ini?')) {
				...
				$.ajax({
					url: '<?php echo base_url();?>C_pusat/deleteLaporanProgram/' + id_program,
					type: 'POST',
					dataType: 'JSON',
					success: function (data) {
						location.reload();
		...
		// 	$.ajax({
		// 		url: '<?php echo base_url();?>C_pusat/getKodeKegiatan',
		// 		data: {
		...
		// 		dataType: 'JSON',
		// 		success: function (data) {
		// 			console.log(data[0].program_kegiatan);

		// 			$('#program_kegiatan').val(data[0].program_kegiatan);
